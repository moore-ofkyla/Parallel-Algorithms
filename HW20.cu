...
Notes:
-Fastest time on my machine 766862 microseconds on N=1024
 -check for power of 2 and size
 -shared memory in force kernal.
 -unrolling loop in the forces did not speed mine up


look into. 
fast math. haven't added because I want to make sure I understand what I need to check.
float3 vs float4
...
{
    __shared__ float3 sharedP[BLOCK_SIZE]; // Shared memory for positions
    __shared__ float sharedM[BLOCK_SIZE];   // Shared memory for masses
    
    int i = threadIdx.x + blockDim.x * blockIdx.x;

    if (i < n) {
        sharedP[threadIdx.x] = p[i]; // store data in shared memory 
        sharedM[threadIdx.x] = m[i];
        ...
        for (int j = 0; j < blockDim.x && j < n; j++) {
            if (i != blockIdx.x * blockDim.x + j) {
                float dx = sharedP[j].x - p[i].x;
                float dy = sharedP[j].y - p[i].y;
                float dz = sharedP[j].z - p[i].z;
                float d2 = dx * dx + dy * dy + dz * dz;
                float d = sqrt(d2);
                float force_mag = (g * m[i] * sharedM[j]) / d2 - (h * m[i] * sharedM[j]) / (d2 * d);
