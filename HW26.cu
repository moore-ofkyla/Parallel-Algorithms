...
void setup()
{
	...
	GridSize.z = 1;
    	...
    	Damp = 0.5;
    	//allocate memory accessible by both the CPU and GPU
		//now we dont need to copy memory back and forth between CPU and GPU
...
void nBody()
{
	...
	while(t < RUN_TIME)
	{
		...
		//might need to sync between getForces and moveBodies...but its working so
