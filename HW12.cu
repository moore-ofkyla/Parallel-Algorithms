...
//constant
__constant__ sphereStruct ConstSphere [NUMSPHERES];//defines a constant memory array of sphereStruct values. to hold our sphere info
...
__global__ void makeSphersBitMap(float *pixels)
{
	...
	{
		hitValue = hit(pixelx, pixely, &dimingValue,ConstSphere[i]);
		...
		if(maxHit < hitValue)
		{
			...
			pixelr = ConstSphere[i].r * dimingValue; 	
			pixelg = ConstSphere[i].g * dimingValue;	
			pixelb = ConstSphere[i].b * dimingValue; 	
