...
int main()
{
	...
	for(int i = 0; i < ENTIRE_DATA_SET; i += DATA_CHUNKS*2)
	{
		...
		cudaMemcpyAsync(B0_GPU, B_CPU+i+DATA_CHUNKS, DATA_CHUNKS*sizeof(float),cudaMemcpyHostToDevice,Stream0);
		cudaErrorCheck(__FILE__, __LINE__);
		...
		//not overlapped version...actually faster... 
		// cudaMemcpyAsync(A0_GPU, A_CPU+i, DATA_CHUNKS*sizeof(float),cudaMemcpyHostToDevice,Stream0);
		// cudaErrorCheck(__FILE__, __LINE__);
	...
	// Make the CPU waits until the Streams have finished before it continues.
